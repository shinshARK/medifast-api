...
-- ----------------------------
INSERT INTO `catatan_dokter` VALUES ('1', 'Anak tubuh panas melebihi 37°C, batuk pilek dan flu, muntah berkepanjangan, jarang buang air kecil, tidak mengeluarkan air mata saat menangis, dan lemas.', 'Mengalami Demam');
INSERT INTO `catatan_dokter` VALUES ('2', 'Nafsu Makan Berkurang, Demam Tinggi mencapai 40°C, Keringat Berlebih, Diare dan Sembelit', 'Mengalami Tipes');
INSERT INTO `catatan_dokter` VALUES ('3', 'Refluks asam lambung, Asma, Alergi,  Infeksi saluran pernapasan atas', 'Mengalami Batuk');
...
INSERT INTO `pasien` VALUES ('3', 'repan dhia', '23', 'laki-laki', 'batuk', 'gelong bawah');
...
CREATE TABLE `resep_digital` (
  ...
  KEY `dokter_keyRESEP` (`id_dokter`),
  KEY `pasien_keyRESEP` (`id_pasien`),
  KEY `obay_keyRESEP` (`id_obat`),
  CONSTRAINT `dokter_keyRESEP` FOREIGN KEY (`id_dokter`) REFERENCES `doctor` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
...
CREATE TABLE `riwayat_transaksi` (
  ...
  CONSTRAINT `cataanD_keyT` FOREIGN KEY (`id_catatan_dokter`) REFERENCES `catatan_dokter` (`id_catatan_dokter`) ON DELETE CASCADE ON UPDATE CASCADE,
